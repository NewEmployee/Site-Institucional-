    ...
    arduino.pipe(new serialport.ReadlineParser({ delimiter: '\r\n' })).on('data', async (data) => {
        ...
        if (HABILITAR_OPERACAO_INSERIR) {
            ...
            if (AMBIENTE == 'producao') {
                ...
                let fk_sensor = Math.floor(Math.random(1) * (4) + 1);
                // Este insert irá inserir os dados na tabela "medida" -> altere se necessário
                // Este insert irá inserir dados de fk_aquario id=1 >> você deve ter o aquario de id 1 cadastrado.
                await poolBancoDados.execute(
                    `INSERT INTO registro_sensor (valor_registro, fk_sensor) VALUES (?,${fk_sensor})`,
                    [lm35Temperatura]
                );
                console.log("valores inseridos no banco: ",lm35Temperatura + ", " + fk_sensor) 
